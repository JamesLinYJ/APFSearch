...
parser.add_argument('--identity', default=os.environ.get('FILESEARCH_SIGN_IDENTITY'), required=not os.environ.get('FILESEARCH_SIGN_IDENTITY'))
args = parser.parse_args()
project = pathlib.Path(__file__).resolve().parents[1]
work = args.work or pathlib.Path(tempfile.mkdtemp(prefix='FileSearch-window-benchmark-'))
...
receipt = {
    ...
    'defaults': 'Unique test bundle identifier; signature identifier is local.filesearch.cli for existing service authentication',
    'compiler_command': command,
}
(app / 'Contents/Resources/build-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
subprocess.run(['codesign', '--force', '--options', 'runtime', '--timestamp=none', '--identifier', 'local.filesearch.cli', '--sign', args.identity, str(app)], check=True)
