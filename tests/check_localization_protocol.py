...
with tempfile.TemporaryDirectory(prefix='FileSearch-wire-localization-') as tmp:
    ...
    subprocess.run(['swiftc', '-module-cache-path', str(tmp/'ModuleCache'), '-swift-version', '5', '-target', 'arm64-apple-macos15.0', str(root/'macos/ApplicationIdentity.swift'), str(root/'macos/SearchProtocol.swift'), str(root/'macos/Localization.swift'), str(root/'macos/SearchClient.swift'), str(root/'tests/LocalizationProtocolTests.swift'), '-o', str(executable)], check=True)
    ...
    report['catalog_languages_verified'] = ['en','zh-Hans','zh-Hant']
