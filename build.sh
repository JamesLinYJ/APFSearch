...
for APFSEARCH_ARCHITECTURE in "${APFSEARCH_ARCHITECTURES[@]}"; do
  ...
  swiftc "${APFSEARCH_FLAGS[@]}" "$APFSEARCH_ROOT/macos/ApplicationIdentity.swift" "$APFSEARCH_ROOT/macos/SearchProtocol.swift" "$APFSEARCH_ROOT/macos/Localization.swift" "$APFSEARCH_ROOT/macos/SearchClient.swift" "$APFSEARCH_ROOT/macos/UpdateManager.swift" "$APFSEARCH_ROOT/macos/UpdateUI.swift" "$APFSEARCH_ROOT/macos/SelectionResolver.swift" "$APFSEARCH_ROOT/macos/FileOperationReview.swift" "$APFSEARCH_ROOT/macos/Application.swift" "$APFSEARCH_ROOT/macos/DuplicateResultsWindow.swift" "$APFSEARCH_ROOT/macos/SettingsWindow.swift" -framework AppKit -framework SwiftUI -framework ServiceManagement -framework Quartz -framework Carbon -framework CryptoKit -o "$APFSEARCH_SLICE/$APFSEARCH_EXECUTABLE"
done
