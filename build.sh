...
swiftc "${FILESEARCH_FLAGS[@]}" "$FILESEARCH_ROOT/macos/ApplicationIdentity.swift" "$FILESEARCH_ROOT/macos/LegacyDataMigration.swift" "$FILESEARCH_ROOT/macos/SearchProtocol.swift" "$FILESEARCH_ROOT/macos/Localization.swift" "$FILESEARCH_ROOT/macos/SearchClient.swift" "$FILESEARCH_ROOT/macos/Application.swift" "$FILESEARCH_ROOT/macos/DuplicateResultsWindow.swift" "$FILESEARCH_ROOT/macos/SettingsWindow.swift" -framework AppKit -framework SwiftUI -framework ServiceManagement -framework Quartz -framework Carbon -o "$FILESEARCH_APP/Contents/MacOS/$FILESEARCH_EXECUTABLE"
