...
fn replay_more_than_two_thousand_changes_is_read_only_and_matches_sql() {
    ...
    store
        .finish_observed(
            &[],
            std::slice::from_ref(&files[3_002].path),
            &RoaringTreemap::new(),
            7,
        )
        .unwrap();
    store
        .prune_namespace_scopes(
            std::slice::from_ref(&files[3_003].path),
            &AtomicBool::new(false),
        )
        .unwrap();
    let aliases: Vec<_> = ["alias-one.txt", "alias-two.txt"]
        .into_iter()
    ...
    let (restored, generation) = store
        .cache_read()
        .expect("bounded cache replay must not fall back to SQL");
    assert_eq!(generation, 2);
    assert_eq!(writes(&store), before, "cache replay wrote SQLite rows");
    assert_eq!(
        std::fs::read(&store.cache_path).unwrap(),
        old_bytes,
        "cache replay rewrote the binary cache"
    );
    let expected = SearchSnapshot::new(store.entries().unwrap(), 2);
    assert_eq!(rows(&restored), rows(&expected));
    assert_eq!(
        paths(&restored, &restored.name_order),
        paths(&expected, &expected.name_order)
    );
    assert_eq!(
        paths(&restored, &restored.path_order),
        paths(&expected, &expected.path_order)
    );
    assert_eq!(
        restored
            .visible_entries()
    ...
    assert_eq!(
        ...
        store.get("content_revision", json!(0)).as_u64().unwrap()
    );
    let before = writes(&store);
    assert_eq!(
        store
            .observe_batch(&files[..2_501], None, None, None)
            .unwrap(),
        0
    );
    assert_eq!(
        writes(&store),
        before,
        "unchanged observations wrote SQLite rows"
    );
...
fn metadata_rollback_restores_journal_and_counter_together() {
    ...
    store
        .connection
        .execute_batch("BEGIN; UPDATE files SET size=size+1 WHERE id=1;")
        .unwrap();
...
fn bounded_reader_never_returns_a_truncated_delta() {
    ...
    store.batch(&files, 1).unwrap();
    assert!(
        change_reader::read(&store.connection, change_reader::Journal::Cache, 2)
            .unwrap()
            .is_none()
    );
    let changes = change_reader::read(&store.connection, change_reader::Journal::Cache, 3)
        .unwrap()
        .unwrap();
